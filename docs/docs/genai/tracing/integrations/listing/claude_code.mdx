...
- Git state of the session's repository, for reproducibility: `mlflow.source.git.commit`,
  `mlflow.source.git.branch`, `mlflow.source.git.repoURL` and `mlflow.source.git.dirty`
  (`"true"` when the checkout had uncommitted changes to tracked files). Sessions outside a
  git repository simply record no git metadata.
